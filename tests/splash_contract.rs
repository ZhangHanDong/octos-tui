...
fn curated_effects_produce_frames_on_virtual_clock() {
    for args in SPLASH_EFFECTS {
        let name = args[0];
        let mut session = SplashSession::new(args, &splash_text(), test_opts(), 80)
...
fn truncated_run_ends_with_full_logo() {
    let text = splash_text();
    let mut session =
        SplashSession::new(&["decrypt"], &text, test_opts(), 80).expect("session builds");
    let mut out: Vec<u8> = Vec::new();
    let mut calls = 0;
    let stats = session
        .run(&mut out, || {
...
fn play_swallows_engine_errors() {
    ...
    let result = SplashSession::new(&["decrypt"], "", test_opts(), 80);
